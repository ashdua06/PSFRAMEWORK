...
public class WFMLeaveRoundDownTest {
	...
	String roundOffDown50WorkSheetName = "round_down_50";
	String roundOffDown25WorkSheetName = "round_down_25";
	...
	// with .50 values
	String roundDown50InsertQuery = "insert Into   TmLeaveBalanceRules values ('ROUND_OFF','ROUND_DOWN',0.5,258,156,1,GETDATE(),1,GETDATE(),1,NULL)";
	// Query to insert row in TmLeaveBalanceRules for ROund OFF using round down
	// with .25 values
	String roundDown25InsertQuery = "insert Into   TmLeaveBalanceRules values ('ROUND_OFF','ROUND_DOWN',0.25,258,156,1,GETDATE(),1,GETDATE(),1,NULL)";
	...
	@Test(priority = 1)
	public void leaveRoundDown50_RejectTest() throws Exception {
		System.out.println(this.getClass().getName());
		DBConnection.executeQuery(deleteLeaveRuleQuery);
		DBConnection.executeQuery(roundDown50InsertQuery);
	...
	@Test(priority = 2)
	public void leaveRoundDown25Test() throws Exception {
		System.out.println(this.getClass().getName());
		SoftAssert leaveRoundUpValidate = new SoftAssert();
		SoftAssert leaveRoundUpReject = new SoftAssert();
		DBConnection.executeQuery(deleteLeaveRuleQuery);
		DBConnection.executeQuery(roundDown25InsertQuery);
		int rowNum = util.getSheetRowNum(roundOffWorkBookName,
				roundOffDown25WorkSheetName);
		int colNum = util.getSheetColNum(roundOffWorkBookName,
				roundOffDown25WorkSheetName);
		String[][] data = new String[rowNum][colNum];
		// get data from sheet
		data = util.getSheetData("Round_Trim_Data.xlsx", "round_down_25");
		for (int i = 1; i < rowNum; i++) {
			DBConnection.executeQuery(leaveDeleteQuery);
			leaveDecimalLessThan50 = false;
			leaveDecimalGreaterThan50 = false;
